...
public class BmiOAuth2User implements OAuth2User {
  ...
  public BmiOAuth2User(Map<String, Object> attributes) {
    Assert.notNull(attributes, "attributes of ir.sadad.util.BmiOAuth2User object must not null");
    this.attributes = attributes;
    this.authorities = extractAuthorities(attributes);
    Assert.notNull(getSsn(), "SSN of ir.sadad.util.BmiOAuth2User object must not null");
    this.name = getSsn();
    this.token = getToken();
  }
